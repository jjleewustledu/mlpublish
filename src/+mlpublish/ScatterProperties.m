classdef ScatterProperties % < mlpublish.PublishProperties
    %% SCATTERPROPERTIES is a registry of objects and data useful for creating figures for publication.
    %
    %  Instantiation:  pp = mlpublish.ScatterProperties(filestem, title, nums)
    %                                                   ^         ^ strings
    %                                                                    ^ # of series
    %
    ...
    properties (Access = 'protected') % default values
		...
		legendLocation_       = 'NorthEastOutside';
    ...
    methods
        ...
        function self = ScatterProperties(filestem, atitle, nums)
            
            %% SCATTERPROPERTIES CTOR  
            %  Usage:  pp = mlpublish.ScatterProperties(filestem, title, nums)
            %                                           ^         ^ strings
            %                                                            ^ # of series
            %
            import mlpublish.*;
            ...
            switch (nargin)
                case 0 % required by OOMatlab; use property initializations
                    self.filestemEps = ['ScatterProperties_ctor_' datestr(now,30)];
                case 1
                ...
                otherwise
                    throw(MException( ...
                       ...
                       ['ScatterProperties.ctor does not support ' num2str(nargin) ' input params']));
            end
            ...
            self.db              = mlfsl.Np797Registry.instance;
            ...
            if (isa(self, 'BlandAltmanProperties'))
                ...
                if (isempty(self.anXLabel))
                    if (strcmp('none', self.rescale))
                        units = mlpublish.ScatterProperties.unitsOfMeasurement({self.xModality self.xMetric});
        ...
        function   yl = get.aYLabel(self)
            ...
            if (isa(self, 'BlandAltmanProperties'))
                ...
                if (isempty(self.aYLabel))
                    if (strcmp('none', self.rescale))
                        units = mlpublish.ScatterProperties.unitsOfMeasurement({self.yModality self.yMetric});
                        if (~isempty(units)); units = [' / ' units]; end
                    else
                        units = '';
                    end
            ...
            if (self.numSeries ~= length(self.markers))
                ...
                if (iscell(self.markers_))
                    ...
                    for i = 1:self.numSeries %#ok<*FORFLG,*PFUNK>
                        self.markers{i} = self.markers_; %#ok<*PFPIE>
                    end
                end
            end
            m = self.markers;
                ...
                switch (self.colorscheme)
                    case 'grey'
                        for r = 1:self.numSeries
                            if (self.numSeries > 1)
                                self.markerColors{r} = mlpublish.ScatterProperties.greyValue( ...
                    ...
                    case 'rgb'
                        for r = 1:self.numSeries
                            if (self.numSeries > 1)
                                self.markerColors{r} = mlpublish.ScatterProperties.rgbWarpedValue( ...
		...
		function self = set.xMetric(self, metr)
            ...
            %
			self.xMetric = mlpublish.ScatterProperties.metricfind(metr);
		end
		...
		function self = set.yMetric(self, metr)
            ...
            %  metr:  metric or units of measurement
            %
			self.yMetric = mlpublish.ScatterProperties.metricfind(metr);
        end
        
        function   ok = checkSelf(self)
            
            %% CHECKSELF checks that all fields of the instantiated ScatterProperties 
            %            are sensible.
            %
            %  Usage:  props = mlpublish.ScatterProperties(...);
    ...
    methods (Static)
        ...
        function  rgb = rgbValue(x, gamma)
            ...
            %% RGBVALUE   
            %  Usage:  rgb = mlpublish.ScatterProperties.rgbValue(x, gamma)
            ...
            switch (nargin)
                ...
                case 1
                    rgb = mlpublish.ScatterProperties.rgbWarpedValue(x, 2);
                case 2
                    rgb = mlpublish.ScatterProperties.rgbWarpedValue(x, 2, gamma);
                otherwise
        ...
        function  rgb = rgbWarpedValue(x, N, gamma)
            ...
            %% RGBWARPEDVALUE   
            %  Usage:  rgb = mlpublish.ScatterProperties.rgbWarpedValue(x [, N [, gamma]])
            ...
            import mlpublish.*;
            rgb = [ ScatterProperties.cauchy(x - gamma, 0.5, gamma) ...
                    ScatterProperties.cauchy(x,         0.5, gamma) ...
                    ScatterProperties.cauchy(x + gamma, 0.5, gamma) ]; 
        ...
        function rgbs = rgbWarpedValues(N, gamma)
            ...
            %  Usage:     rgbs = mlpublish.ScatterProperties.rgbWarpedValues(N [, gamma])
            ...
            rgbs = cell(N,1);
            for x = 1:N %#ok<*FORPF>
                rgbs{x} = mlpublish.ScatterProperties.rgbWarpedValue(x, N, gamma);
            end
        ...
        function units = unitsOfMeasurement(tags)
            ...
            %  Usage:  units = mlpublish.ScatterProperties.unitsOfMeasurement(tags)
